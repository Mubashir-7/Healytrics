...
Original file is located at
    https://colab.research.google.com/drive/1G87IQhJ3IJ4AyU77a5PKSXfhlhGYxkDk
"""
...
rf_model.predict(X_test.iloc[0].values.reshape(1,-1))
...
prec = pd.read_csv('/content/precautions_df.csv')
prec

wrkout = pd.read_csv('/content/workout_df.csv')
wrkout
...
def helper(dis):
  ...
  d = diet[diet['Disease'] == dis]['Diet']
  d = [d for d in d.values]

  wrkout = workout[workout['disease'] == dis]['workout']

  return description, pre, med, d, wrkout



def get_predicted_value(patient_symptoms):
    input_vector = np.zeros(symptoms_dict) #len of symtoms_dict

    for items in patient_symptoms:
      index_vector = [symtoms_dict[item]] = 1
...
description,pre,med,d,wrkout = helper(predicted_disease)
...
print("------------------------Precaution-----------------------------")
for idx, pi in enumerate(pre[0], 1):
    print(f"{idx}. {pi}")
print()

print("------------------------Medication-----------------------------")
for idx, mi in enumerate(med, 1):
    print(f"{idx}. {mi}")
print()

print("------------------------Diet-----------------------------------")
for idx, di in enumerate(d, 1):
    print(f"{idx}. {di}")
print()

print("------------------------Workout-------------------------------")
for idx, wo in enumerate(wrkout, 1):
    print(f"{idx}. {wo}")